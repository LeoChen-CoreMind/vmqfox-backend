...
class QrcodeServer
{
    ...
    public function createQrcode($content) {
        ...
        if (!$usePng && !class_exists(\SimpleXMLElement::class)) {
            throw new \RuntimeException('PHP XML/SimpleXML 扩展未启用，无法生成无 GD 版二维码');
        }
        $writer = $usePng ? new PngWriter() : new SvgWriter();
        $imageType = $usePng ? 'png' : 'svg+xml';
        $cacheExtension = $usePng ? 'png' : 'svg';
        ...
        $cacheFile = $runtimePath . 'qrcode/' . $cacheKey . '.' . $cacheExtension;
        ...
        if (file_exists($cacheFile)) {
            return 'data:image/' . $imageType . ';base64,' . base64_encode(file_get_contents($cacheFile));
        }
