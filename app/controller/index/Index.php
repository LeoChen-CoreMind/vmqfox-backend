...
';
    ...
    public function appPush()
    {
        ...
        if ($key === '') {
            return json($this->getReturn(-1, "系统密钥未设置"));
        }
        if (!hash_equals(md5($type . $price . $t . $key), (string) $sign)) {
    ...
    public function closeOrder()
    {
        ...
        $sign = trim((string) Request::param('sign', ''));
        if ($orderId === '' || $sign === '') {
            return json($this->getReturn(-1, '订单ID和签名不能为空'));
        }

        $key = (string) Db::name('setting')->where('vkey', 'key')->value('vvalue');
        if (!hash_equals(md5($orderId . $key), $sign)) {
            return json($this->getReturn(-1, '签名校验不通过'));
        }
    ...
    {
        $timestamp = trim((string) Request::param('t', ''));
        $sign = trim((string) Request::param('sign', ''));
        if ($timestamp === '' || $sign === '') {
